...
def get_top_k(prompt: str, k=10):
    ...
    with psycopg.connect("user=postgres") as conn:
        with conn.cursor() as cur:
            res = cur.execute(
                f"""SELECT *, embedding <-> %s::vector AS similarity FROM 
                                {EMBEDDING_TABLE} ORDER BY similarity LIMIT %s""",
                (prompt, k),
            ).fetchall()
            return [{"score": record[4], "source_id": record[1]} for record in res]
...
class QueryMaker:
    ...
    def query(
        self, prompt: str, developer_prompt="", k=10, temperature=0, model="gpt-4o-mini"
    ):
        ...
        completion = self.client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": developer_prompt},  # personality
            ],
            temperature=temperature,  # how creative where 0 is deterministic
            stream=True,
        )
        return {"text": completion, "ids": ids, "resumes": resumes}
